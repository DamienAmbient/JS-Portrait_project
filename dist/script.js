...
var drop = function drop() {
  ...
  ["dragenter", "dragleave", "dragover", "over"].forEach(function (eventName) {
...
var scrolling = function scrolling(upSelector) {
  ...
  var calcScroll = function calcScroll() {
    upElem.addEventListener("click", function (event) {
      var scrollTop = Math.round(body.scrollTo || element.scrollTop);
  ...
  var smoothScroll = function smoothScroll(from, to, hash) {
    ...
    var move = setInterval(function () {
      ...
      if (prevScrollTop === scrollTop || to > from && scrollTop >= 0 || to < from && scrollTop <= to) {
        clearInterval(move);
